...
def load_geojson_from_github(url):
    try:
        ...
        return gpd.read_file(geojson_bytes)
    except Exception as e:
        st.error(f"No se pudo cargar el GeoJSON desde GitHub: {e}")
        return None
...
def plot_scatter(df):
    ...
    fig = px.scatter(promedios, x="trabajador", y="venta_prom", color="año",
                     symbol="año", size="experiencia", hover_data=["provincia"],
                     labels={"trabajador": "Promedio de Trabajadores",
...
def plot_bars(df):
    ...
    fig = px.bar(ventas, x="provincia", y="venta_millones", color="año", barmode="group",
...
def plot_map(gdf, df, year_selected, provincias_sel):
    ...
    df_year = df[(df["año"] == year_selected) & (df["provincia"].isin(provincias_sel))]
    ...
    merged.plot(column="venta_plot", cmap="OrRd", linewidth=0.8, ax=ax, edgecolor="0.8",
                legend=True, legend_kwds={"label": "Ventas (Millones de S/.)", "orientation": "vertical"},
                missing_kwds={"color": "#f0f0f0", "edgecolor": "0.8", "label": "Sin datos"})
    ax.axis("off")
    st.pyplot(fig)
...
# -----------------------------
def main():
    # Cargar datos CSV desde la carpeta 'data' de GitHub (puedes ajustar si los subes a GitHub también)
    ...
    # Selector de año para TODOS los gráficos
    year_selected = st.selectbox("Selecciona el año para todos los gráficos", [2022, 2023, 2024])
    filtered_df = combined_df[(combined_df["año"] == year_selected) & (combined_df["provincia"].isin(selected_provinces))]
    ...
    if gdf is not None:
        plot_map(gdf, combined_df, year_selected, selected_provinces)
